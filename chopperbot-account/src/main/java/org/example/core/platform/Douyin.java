...
public class Douyin implements PlatformOperation {
    ...
    @Override
    public Set<org.openqa.selenium.Cookie> login(String id, String username) {
        ...
        try {
            ...
            System.out.println("请在30秒内完成抖音登录操作...");
            Thread.sleep(60000L); // 等待用户手动登录
            ...
            System.err.println("抖音登录过程中发生错误: " + e.getMessage());
            e.printStackTrace();
            throw new RuntimeException("抖音登录失败: " + e.getMessage(), e);
        } finally {
            // 确保资源被正确释放
    ...
    private boolean isLoginSuccessful(WebDriver driver) {
        try {
            ...
            try {
                // 尝试查找用户头像或用户名相关元素
                WebElement userElement = driver.findElement(By.cssSelector(".avatar, .user, .login, .username, .user-name"));
                if (userElement != null && userElement.isDisplayed()) {
                    System.out.println("通过用户元素检测到已登录");
                    return true;
                }
            } catch (Exception e) {
                System.out.println("用户元素检测失败: " + e.getMessage());
            }
